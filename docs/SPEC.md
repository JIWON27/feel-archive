...
> **문서 버전**: 1.0.0
> **작성일**: 2026-03-09
> **최종 동기화**: 2026-03-10 (v1.0.0 구현 기준)
> **서비스명**: Feel-Archive
...
- **위치 노출 수준**: ⚠️ v1.0.0 미구현 (백엔드 LocationRequest에 정밀도 필드 없음)
...
- 감정별 필터
- 키워드 검색 (내용 기준)
- **최신순** (`LATEST`): 작성 시간 기준 최신순 (기본값)
- **오래된순** (`OLDEST`): 작성 시간 기준 오래된순
- **인기순** (`POPULAR`): 좋아요 수 × 1 + 스크랩 수 × 3 합산 점수 기준
- **좋아요순** (`LIKE`): 좋아요 수(likeCount) 단독 기준
...
#### 타임캡슐 상세 페이지 UI
- **잠금(LOCKED) 상태**: 다크 배경 봉인 UI + 실시간 카운트다운 표시 (일/시간/분/초, 1초마다 갱신)
  - 카운트다운이 0이 되면 "곧 열립니다..." 문구 표시 (스케줄러가 처리할 때까지 대기)
- **열림(OPENED) 상태**: 감정 이모지 배너 + 내용 + 이미지 + 위치 표시
- **수정 페이지**: `/timecapsule/{id}/edit` (작성 후 30분 이내 + LOCKED 상태만 접근 가능)
  - 감정, 내용, 열릴 날짜/시간 수정 가능
  - 수정 가능 시간 초과 시 접근 불가 안내 페이지 표시

---

### 2.4 월간 리포트 (Report)

> ⚠️ **v1.0.0 미구현** — 백엔드 API 및 프론트엔드 페이지 모두 미구현. 추후 버전에서 추가 예정.
...
| 이벤트 타입 | `connect` (연결 확인), `heartbeat` (연결 유지), `time-capsule` (알림) |
...
| 토큰 | 저장 위치 | 목적 |
|------|-----------|------|
| Access Token | Zustand 메모리 (JS 변수) | XSS 보호 - 브라우저 스토리지 미사용 |
| Refresh Token | HttpOnly Cookie | CSRF 보호 - JS 접근 불가 |

- **Silent Refresh**: 앱 시작(마운트) 시 `POST /api/v1/token/reIssue`를 자동 호출하여 Refresh Token으로 인증 상태 복원. 페이지 새로고침 후에도 재로그인 불필요.
- **401 처리**: API 호출 중 401 발생 시 Axios interceptor가 자동으로 토큰 재발급 후 원래 요청 재시도
- **로그아웃 시**: Zustand store 초기화 (메모리에서 Access Token 제거), 서버 측 Refresh Token 쿠키 무효화
...
| 스타일링 | Tailwind CSS |
| 상태 관리 | Zustand |
| 서버 상태 / 비동기 | TanStack Query (React Query) + Axios |
...
| 항목 | 선택 |
|------|------|
| 언어/프레임워크 | Java 17 / Spring Boot 3.4.x |
| 데이터베이스 | MySQL 8.4 (공간 데이터 지원) |
| ORM | Spring Data JPA + QueryDSL |
| 마이그레이션 | Flyway |
| 캐싱 | Redis |
| 배치 | Spring Batch ⚠️ v1.0.0 미구현 (스켈레톤만 존재) |

#### 인프라
| 항목 | 선택 |
|------|------|
| 배포 환경 | AWS (EC2, ECR, CodeBuild, CodeDeploy) |
| 이미지 저장 | AWS S3 |
| CI/CD | ✅ 구축 완료 (백엔드: CodeBuild → ECR → CodeDeploy / 프론트엔드: Vercel 자동 배포) |
...
#### Spring Batch
> ⚠️ **v1.0.0 미구현** — BatchApplication 스켈레톤만 존재. 실제 Job 없음.
- **활용 시나리오**:
...
### 4.3 프론트엔드 구현 방향
- **수준**: Next.js 14 App Router 기반 CSR 페이지 구성
- **인증**: JWT Access Token(Zustand 메모리) + Refresh Token(HttpOnly Cookie) + Silent Refresh
- **API 통신**: Axios 인스턴스 + 401 자동 재발급 인터셉터
- **프록시**: `next.config.mjs`에서 `/api/*` → `NEXT_PUBLIC_API_URL/api/*` 리라이트
...
| `/login` | 로그인 |
| `/signup` | 회원가입 |
| `/home` | 홈 (지도 + 아카이브 리스트, 2단 레이아웃) |
| `/archives` | 아카이브 피드 목록 |
| `/archives/new` | 아카이브 작성 |
| `/archives/[id]` | 아카이브 상세 |
| `/archives/[id]/edit` | 아카이브 수정 |
| `/timecapsule/new` | 타임캡슐 작성 |
| `/timecapsule/[id]` | 타임캡슐 상세 (LOCKED/OPENED 상태별 UI) |
| `/timecapsule/[id]/edit` | 타임캡슐 수정 (30분 이내만 접근 가능) |
| `/notifications` | 알림 목록 |
| `/my/profile` | 마이페이지 (프로필, 비밀번호, 이메일 알림, 회원탈퇴) |
| `/my/archives` | 내 아카이브 목록 |
| `/my/scraps` | 스크랩 목록 |
| `/my/timecapsules` | 내 타임캡슐 목록 |
...
// 총 9가지 감정. GET /api/v1/emotions 로 목록 조회 가능 (name, label, sortOrder 반환)
```
...
DELETE /api/v1/users                                    회원탈퇴 (비밀번호 확인 후 처리)
...
GET    /api/v1/notifications/subscribe    SSE 구독 (실시간 알림 수신)
                                           ...
                                           Events: connect, heartbeat (30초마다), time-capsule
```
...
## 10. 미확정/미구현 사항 (v1.0.0 기준)

- [x] ~~상세 페이지 구성 및 화면 설계~~ → 5.4 참고
- [x] ~~감정 태그 최종 목록 확정~~ → 9가지 확정 (6.2 참고)
- [x] ~~캐싱 전략 상세 설계~~ → Redis Sorted Set (감정 날씨), 구현 완료
- [x] ~~AWS 인프라 구성 상세~~ → EC2 + ECR + CodeBuild + CodeDeploy + SSM + S3, 구축 완료
- [ ] 배치 작업 구현 (Soft Delete 정리) — Spring Batch 스켈레톤만 존재
- [ ] 월간 리포트 기능 — API 및 페이지 미구현
- [ ] 위치 정밀도 선택 — LocationRequest에 정밀도 필드 미포함
...
- **최종 선택**: AWS S3 ✅ 구현 완료
- **변경 이유**:
  - AWS 인프라 구성 시 S3 연동으로 전환
  - 백엔드: Spring Boot에서 S3 SDK로 업로드, `AWS_S3_BUCKET` 환경변수로 버킷 관리
  - 이미지 URL: `/api/v1/archives/{id}/images/{imageId}` (JWT 인증 required)
  - 프론트엔드: `AuthImage` 컴포넌트로 JWT 포함 fetch 후 Blob URL 변환 방식으로 표시
