...
⠀
### 출력 산출물 (반드시 이 2개만)
**1.** **IMPLEMENTATION_PLAN.md** (프론트엔드 계획 문서) <br>
**2.** **TASK_LIST.json** (프론트엔드 Task List JSON)
⠀
### 세션 시작 체크 (명령어 없이, 상태 점검 개념으로만)
아래를 **문서/정보 기반으로 확인**하고, 불충분하면 “알 수 없음/추가 정보 필요”로 표시한다. <br>
...
**0.** **자료 읽기/정리**
  ...
  * 목표(예: MVP 배포/핵심 기능 구현/포트폴리오 완성)와 기능 요구사항을 “필수/선택”으로 분류
...
### 우선순위 결정 기준
**1.** **Blocking dependencies** (다른 작업이 의존하는 선행 조건) <br>
**2.** **Core Features** (핵심 가치 구현: API 통합, 사용자 플로우, MVP 필수 UI, 사용자 경험을 만드는 핵심)
**3.** **Infrastructure & Stability** (인증 상태 관리, 지도 통합, 배포 설정, 에러 처리)
**4.** **Polish** (리팩토링/디자인 미세조정/성능) <br>
⠀
### Ultimate Goal (제품 목표)
개인 감정/위치 아카이빙 서비스 "Feel-Archive" 프론트엔드:
* 3개월 내: **실사용자 50명**
* 필요 UI 요소:
  * 감정/경험 기록 폼 (작성/수정/삭제) + 위치 선택 (GPS/지도)
  * 타임캡슐 작성 및 공개 알림 UI
  * 회원가입/로그인 페이지 및 인증 상태 관리
  * 마이페이지 (기록 모아보기 + 월간 리포트 차트)
⠀
### 엄격한 제약
